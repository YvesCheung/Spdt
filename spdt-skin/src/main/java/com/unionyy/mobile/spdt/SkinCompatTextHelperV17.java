...
public class SkinCompatTextHelperV17 extends SkinCompatTextHelper {
    ...
    public void loadFromAttributes(AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, skin.support.R.styleable.SkinCompatTextHelper,
                defStyleAttr, 0);
        if (a.hasValue(skin.support.R.styleable.SkinCompatTextHelper_android_drawableStart)) {
            mDrawableStartResId = a.getResourceId(skin.support.R.styleable.SkinCompatTextHelper_android_drawableStart, INVALID_ID);
            mDrawableStartResId = SkinCompatHelper.checkResourceId(mDrawableStartResId);
        }
        if (a.hasValue(skin.support.R.styleable.SkinCompatTextHelper_android_drawableEnd)) {
            mDrawableEndResId = a.getResourceId(skin.support.R.styleable.SkinCompatTextHelper_android_drawableEnd, INVALID_ID);
    ...
    protected void applyCompoundDrawablesRelativeResource() {
        ...
        if (mDrawableStartResId != INVALID_ID) {
            drawableStart = SkinCompatResources.getDrawable(mView.getContext(), mDrawableStartResId);
        }
        if (drawableStart == null) {
            drawableStart = drawableLeft;
        }
        if (mDrawableEndResId != INVALID_ID) {
            drawableEnd = SkinCompatResources.getDrawable(mView.getContext(), mDrawableEndResId);
        }
        if (drawableEnd == null) {
